	...
	private void Swap(TreeNode<P, V> actualNode) {
		if (actualNode != null) {
			...
			if (actualNode.get_parent() != null) { // is not the root
				...
				int result = _priorityComparator.compare(actualNode.get_priority(),
	...
	public V remove() {
		...
		while (actual != null) {
			...
			if (actualHasLeftChild && actualHasRightChild) { // Tiene a los 2 hijos
				...
				int result = _priorityComparator.compare(actual.get_left().get_priority(),
						actual.get_right().get_priority());
